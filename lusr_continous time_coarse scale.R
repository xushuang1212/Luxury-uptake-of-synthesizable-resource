...
#Coarse scale nutrient levels
Trp <- c(0.05, 0.1, 0.25, 0.5, 1) # tryptophan fold
Leu <- c(0.05, 0.1, 0.25, 1) # leucine fold

# change unit to for X to mg/L
Trp <- 73.4*Trp  #concetration of nutients (mg/L)
Leu <- 367*Leu # concentration of leucine (mg/L) 

Clusr <- 0 # coefficient for LUSR
...
for (i in 1:length(Leu)) {
  ...
  for (j in 1:length(Trp)) {
    ...
    for (m in 1:length(freq)) {
      ...
      # parameters: a named vector for course scale
      parms <- c(Rmax1 = Rm1[i], Rmax2 = Rm2[j],
                 Km1 = K1[i], Km2 = K2[j],
                 Y1leu = Y1[i], Y2leu = Y1[i],
                 Y1trp = Y2[j], Y2trp = Y2[j])
...
# library(xlsx)
# write.csv(finaldynamic, "coarsefinal_lusr0.csv")
# write.csv(finaldynamic, "coarsefinal_lusr05.csv")
# write.csv(finaldynamic, "coarsefinal_lusr1.csv")
